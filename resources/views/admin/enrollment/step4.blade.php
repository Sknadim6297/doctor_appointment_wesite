...
@section('content')
@php
    $draftStep4 = $draftStep4 ?? [];
    $defaultDate = old('post_doc_date', data_get($draftStep4, 'post_doc_date') ?: now()->format('d/m/Y'));
    $defaultReceivedDate = old('post_doc_recieved_date', data_get($draftStep4, 'post_doc_recieved_date') ?: now()->addDay()->format('d/m/Y'));
...
<section class="mx-auto max-w-4xl">
    <div class="mb-5 rounded-2xl border border-slate-200 bg-white px-5 py-4 shadow-sm">
        <p class="text-xs font-semibold uppercase tracking-[0.18em] text-blue-600">Step 4 of doctor enrollment</p>
    ...
    @if($latestPolicy)
        <div class="mb-5 rounded-2xl border border-emerald-200 bg-emerald-50 px-5 py-4 text-sm text-emerald-900">
            <p class="font-semibold">Policy Received has been submitted in Step 3.</p>
            <p class="mt-1">Latest policy no: {{ $latestPolicy->policy_no ?: 'N/A' }}, receive date: {{ optional($latestPolicy->receive_date)->format('d/m/Y') ?: 'N/A' }}.</p>
        </div>
    @endif
    ...
    <div class="rounded-3xl border border-slate-200 bg-white p-6 shadow-lg">
        <form action="{{ route('admin.posts.store') }}" method="post" enctype="multipart/form-data" id="post_upload_form" class="space-y-6">
            ...
            <input type="hidden" name="workflow_enrollment_id" id="workflow_enrollment_id_step4" value="{{ $enrollment->id }}">
            <input type="hidden" name="return_to" value="{{ route('admin.enrollment.success', $enrollment->id) }}">
            <input type="hidden" name="finalize_enrollment_flow" value="1">
            ...
            <div class="grid gap-5 md:grid-cols-2">
                <div class="form-group">
                    ...
                    <input type="text" id="post_doc_date" name="post_doc_date" value="{{ $defaultDate }}" class="w-full rounded-xl border border-slate-300 px-4 py-3 text-sm outline-none transition focus:border-blue-500 focus:ring-4 focus:ring-blue-100" autocomplete="off" required>
                </div>
                ...
                <div class="form-group">
                    ...
                    <input type="text" id="post_doc_consignment_no" name="post_doc_consignment_no" value="{{ $defaultConsignment }}" class="w-full rounded-xl border border-slate-300 px-4 py-3 text-sm outline-none transition focus:border-blue-500 focus:ring-4 focus:ring-blue-100" required>
                </div>
                ...
                <div class="form-group">
                    ...
                    <input type="text" id="post_doc_recieved_date" name="post_doc_recieved_date" value="{{ $defaultReceivedDate }}" class="w-full rounded-xl border border-slate-300 px-4 py-3 text-sm outline-none transition focus:border-blue-500 focus:ring-4 focus:ring-blue-100" autocomplete="off" required>
                </div>

                <div class="form-group">
                    <label class="mb-2 block text-sm font-semibold text-slate-700" for="post_doc_recieved_by">Received by</label>
                    <input type="text" id="post_doc_recieved_by" name="post_doc_recieved_by" value="{{ $defaultReceivedBy }}" class="w-full rounded-xl border border-slate-300 px-4 py-3 text-sm outline-none transition focus:border-blue-500 focus:ring-4 focus:ring-blue-100">
                </div>
            ...
            <div class="flex flex-wrap items-center justify-end gap-3 border-t border-slate-200 pt-5">
                <a href="{{ route('admin.enrollment.step3', $enrollment) }}" class="rounded-xl border border-slate-300 px-4 py-2.5 text-sm font-semibold text-slate-700 hover:bg-slate-50">Back to Step 3</a>
...
@push('scripts')
    ...
    <script>
        ...
        (function () {
            ...
            const workflowField = document.getElementById('workflow_enrollment_id_step4');
