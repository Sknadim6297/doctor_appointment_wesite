...
@section('title', 'High Risk Plan')
@section('page-title', 'High Risk Plan Management')
...
<section class="section-card" x-data="{
    ...
    <div class="mb-5 flex flex-wrap items-center justify-between gap-3">
        <h3 class="section-title mb-0">High risk plan ({{ $totalPlans ?? 0 }})</h3>
        <div class="flex flex-wrap items-center gap-2">
            <form method="GET" action="{{ route('admin.high-risk-plans') }}" class="flex items-center gap-2">
                <input
                    type="text"
                    name="search"
                    value="{{ $search ?? '' }}"
                    placeholder="Search by coverage or yearly amount"
                    class="master-search-input"
                >
                <button type="submit" class="btn btn-primary">Search</button>
                @if(!empty($search))
                    <a href="{{ route('admin.high-risk-plans') }}" class="btn btn-default">Clear</a>
                @endif
            </form>
    ...
    <div class="overflow-x-auto">
        <table class="data-table">
            ...
            <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            <div class="flex flex-wrap items-center gap-2">
                                ...
                                <form method="POST" action="{{ route('admin.high-risk-plans.destroy', $plan) }}" onsubmit="return confirm('Delete this plan?');">
                                    @csrf
                                    @method('DELETE')
                ...
                @empty
                    <tr>
                        <td colspan="9" class="text-center text-slate-500">No high risk plans found.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
    ...
    <div class="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 px-4" x-show="addModalOpen" x-transition.opacity x-cloak>
        <div class="modal-content w-full max-w-2xl" @click.away="addModalOpen = false">
            <form action="{{ route('admin.high-risk-plans.store') }}" method="POST" class="form-horizontal" id="add_plan_form" onsubmit="return addplan()">
                @csrf
    ...
    <div class="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 px-4" x-show="editModalOpen" x-transition.opacity x-cloak>
        <div class="modal-content w-full max-w-2xl" @click.away="editModalOpen = false">
            <form :action="`{{ url('/admin/high-risk-plans') }}/${editId}`" method="POST" class="form-horizontal" id="edit_plan_form" onsubmit="return editplan()">
                @csrf
                @method('PUT')
                <div class="modal-header">
                    <h3>Edit Plan</h3>
