...
foreach ($lines as $line) {
    ...
    foreach ($row['message']['content'] ?? [] as $part) {
        $candidate = $part['text'] ?? '';
        if (($part['type'] ?? '') !== 'text' || ! str_contains($candidate, 'INSERT INTO `tbl_employee_salary`')) {
            continue;
        }
...
if ($text === '') {
    fwrite(STDERR, "Employee salary SQL dump not found in transcript.\n");
    exit(1);
}

$start = strpos($text, 'INSERT INTO `tbl_employee_salary`');
if ($start === false) {
    fwrite(STDERR, "INSERT statement not found.\n");
    exit(1);
}
...
$out = dirname(__DIR__) . '/storage/app/legacy_tbl_employee_salary.sql';
